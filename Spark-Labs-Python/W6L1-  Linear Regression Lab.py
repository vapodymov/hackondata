# Databricks notebook source exported at Sun, 18 Sep 2016 17:19:43 UTC
# MAGIC %md
# MAGIC <a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/4.0/"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-nd/4.0/88x31.png" /></a><br />This work is licensed under a <a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/4.0/">Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License</a>.
# MAGIC 
# MAGIC Based on the EdX Databricks Course
...
# MAGIC %md
# MAGIC **WARNING:** Before executing the following commands for the first time, you need to follow the instructions in the mount_data notebook in this folder, or download and execute [this](https://databricks-staging-cloudfront.staging.cloud.databricks.com/public/c65da9a2fa40e45a2028cddebe45b54c/8637560089690848/3241874368617432/6977722904629137/latest.html), notebook, to mount input datasets.
...
# MAGIC  
# MAGIC > Note:
# MAGIC > * `split()` can be called directly on a `unicode` or `str` object.  For example, `u'split,me'.split(',')` returns `[u'split', u'me']`.
...
def parsePoint(line):
    ...
    label = float(substrings[0])
    features = substrings[1:]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    MSE = labelsAndPreds.map(lambda x: squaredError(x[0], x[1])).mean()
    return np.sqrt(MSE)
...
Test.assertTrue(np.allclose(rmseValLR1, 19.8730701066) or np.allclose(rmseValLR1, 19.6912473416),
                'incorrect value for rmseValLR1')
...
Test.assertTrue(np.allclose(actualResults, expectedResults) or np.allclose(actualResults, expectedResultsE),
                'incorrect value for modelRMSEs')
...
Test.assertTrue(np.allclose(rmseValInteract, 15.9963259256) or np.allclose(rmseValInteract, 15.6894664683),
                'incorrect value for rmseValInteract')
