# Databricks notebook source exported at Sun, 18 Sep 2016 17:18:41 UTC
# MAGIC %md
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    NumAvgTuple = (IDandRatingsTuple[0], (len(IDandRatingsTuple[1]), sum(IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1]))))                  
    return NumAvgTuple
...
movieNameWithAvgRatingsRDD = (moviesRDD.join(movieIDsWithRatingsRDD)
                              .map(lambda x: x[1]).map(getCountsAndAverages)
                              .map(lambda y: (y[1][1], y[0], y[1][0])))       ### (lambda y: 'average rating', 'movie name(ID)', 'number of ratings')
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD.join(actualReformattedRDD)
                        .map(lambda x: (x[1][0] - x[1][1]) * (x[1][0] - x[1][1])))
...
# the average rating across all movies in that training dataset
trainingAvgRating = trainingRDD.map(lambda (x, y, z): z).reduce(lambda x, y: x + y) / float(trainingRDD.count())
...
myRatedMovies = [
     ...
     (myUserID, 1088, 5), (myUserID, 1171, 3), (myUserID, 1047, 1), (myUserID, 1195, 5), (myUserID, 831, 1),
     (myUserID, 503, 1), (myUserID, 651, 1), (myUserID, 1447, 4), (myUserID, 1110, 3), (myUserID, 700, 1)
    ]
...
ratingsWithNamesRDD = (predictedWithCountsRDD.join(moviesRDD).map(lambda x: (x[1][0][0], x[1][1])))
